...
export default function ImageUpload({image_from_server}){
    ...
    async function uploadFile(){
        ...
        let dp_file_name = await handleFileUpload(file, "display_picture", (progress)=>{ console.log("Progress : ", progress) }, false)
        
        if(dp_file_name !== false){
            let response = await post(CUSTOMER_APIS.UPDATE_CUSTOMER_PROFILE_PIC,{ "file_name": dp_file_name })
